...
export const useWorkspaceCreator = create<ListStore>()((set) => ({
    ...
    addtWorkspaceCreatorData: (id) =>
        set((state) => ({
            workspaceCreatorData: [
                ...state.workspaceCreatorData,
                {id, date: new Date(), areas: [], status: 'incomplete'},
            ],
        })),
    ...
    addArea: (workspaceId, item) =>
        set((state) => ({
            workspaceCreatorData: state.workspaceCreatorData.map((workspace) =>
                workspace.id === workspaceId
                    ? {...workspace, areas: [...workspace.areas, item]}
                    : workspace,
            ),
        })),
    addItem: (workspaceId, areaId, title) =>
        set((state) => ({
            workspaceCreatorData: state.workspaceCreatorData.map((workspace) =>
                workspace.id === workspaceId
                    ? {
