...
export const WorkspaceArea = (props: IWorkspaceArea & {numberId: number}) => {
    ...
    const handleEnter = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        if (e.key === 'Enter' && currentDiscassionId) {
            ...
            update(ref(db, 'workspaces/' + currentDiscassionId + '/areas/' + numberId + `/items/${items?.length ?? 0}`), { id: v4(), text, votes: 0 });
    ...
    return (
        <Styled.WorkspaceArea>
            <Styled.WorkspaceAreaHeader>{title}
                <IconButton onClick={() => setShow(true)}><Add /></IconButton>
            </Styled.WorkspaceAreaHeader>
            <Styled.WorkspaceAreaBody>
                {items?.map(item => <WorkspaceAreaItem key={item.id} {...item} />)}
                {show && <WorkspaceAreaItem type={'input'} input={<Styled.Input value={text} onChange={handleChange} onBlur={handleBlur} onKeyDown={handleEnter} />} />}
            </Styled.WorkspaceAreaBody>
        </Styled.WorkspaceArea>
    )
};