...
export const DiscassionsList = () => {
    ...
    useEffect(()=>{
        if(currentDiscassionId){
            navigate({pathname:'/', search: createSearchParams({id:currentDiscassionId ?? ''}).toString()})
        }
    ...
    return(
        <Styled.DiscassionsList>
            {discassionsData.map((item) => <WorkspaceListItem key={item.id} setCurrent = {()=>setCurrent(item.id)} item={item}/>)}
        </Styled.DiscassionsList>
    )
}